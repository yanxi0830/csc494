...
def format_problem(w, t, c, name):
    ...
    # PROBLEM DEFINITION
    acc = list(['(define (problem {})'.format(name), '\t(:domain construction)'])
    ...
    acc.append('\t\t' + ' '.join(wood_parts + ['-', 'part']))
    acc.append('\t\t' + ' '.join(glass_parts + ['-', 'part']))
    acc.append('\t\t' + ' '.join(clay_parts + ['-', 'part']))
    ...
    for o in wood_parts:
        acc.append('\t\t(WOOD {})'.format(o))
    for o in glass_parts:
        acc.append('\t\t(GLASS {})'.format(o))
    for o in clay_parts:
        acc.append('\t\t(CLAY {})'.format(o))
...
def generate_instance(w, t, c):
    name = 'construction-t{}-c{}-w{}'.format(t, c, w)
