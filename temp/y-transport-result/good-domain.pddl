...
(:requirements :typing)
(:types location locatable capacity-number  - object truck package  - locatable )
(:predicates   (monitor ?p - package)
  (server ?p - package)
  (road ?loc1 - location ?loc2 - location)
...
(:action unload-goal
	:parameters (?t - truck ?l - location ?m - package ?s - package ?n1 - capacity-number ?n2 - capacity-number ?n3 - capacity-number)
	:precondition (and
	  (monitor ?m)
	  (server ?s)
	  (goal ?l)
